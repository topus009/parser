...
function addHeader(sheet, title, colsCount, second_title) {
...
const buildReport = (data, lists, second_title) => {
    const sheet = workbook.addWorksheet('Отчет');
    const colsCount = lists.length;
    addHeader(sheet, title, colsCount, second_title);
    addTableHeader(sheet, lists);
    addTable(sheet, data, lists);
    workbook.xlsx.writeFile(`../${title} ${second_title}.xlsx`);
}
//================== heplers =================
